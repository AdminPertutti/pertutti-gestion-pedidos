...
function initAutocomplete() {
  ...
  autocomplete = new google.maps.places.Autocomplete(
    document.getElementById("dirSource"),
    { types: ["geocode"] }
  );
...
var componentForm = {
  ...
  for (const component of place.address_components) {
    const addressType = component.types[0];
      ...
      const val = component[componentForm["street_number"]];
      document.getElementById("street_number").value = val;
    }
    if (addressType == "route") {
    const val = component[componentForm["route"]];
    document.getElementById("route").value = val;
  }
  if (addressType == "locality") {
  const val = component[componentForm["locality"]];
  document.getElementById("localidad").value = val;
}
document.getElementById('detalle_direccion').focus();
  }
}
...
function codeAddress() {
  ...
  $(document).ready(function() {
    ...
    $('#calcular').on('click' , function(){
      ...
      //alert(direccion);
      var request = $.ajax('/assets/dist/js/mapa.php',
        {
        type: 'POST',
